...
class CatagorListAdapter(val list: ArrayList<CatagoryList>,val context: Context):RecyclerView.Adapter<CatagorListAdapter.ViewHolder> (){
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.textView.text=list.get(position).category_name
        holder.cardView.setOnClickListener(View.OnClickListener {

            context.startActivity(Intent(context,SubcategoryActivity::class.java))
            SharedPrefManager.getInstance(context)?.selectCatagory(list.get(position).id)
            SharedPrefManager.getInstance(context)?.selectCatagoryName(list.get(position).category_name)
